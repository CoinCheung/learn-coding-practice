...
class GameField():
    def __init__(self, screen):
        ...
        self.actiondict = dict(
            zip('wsadrqWSADRQ', ('up', 'down', 'left', 'right', 'restart',
                                 'quit') * 2))
    ...
    def draw(self):
        ...
        def draw_row(row):
            line = ''.join('|{:^6}'.format(num) if num > 0 else '|      '
                           for num in row) + '|' + os.linesep
            self.stdscr.addstr(line)
            print("good")
    ...
    def move(self, direction):
                ...
                for j in range(0, self.height - 1):
                    for k in range(j + 1, self.height):
                        if self.field[k][i] == 0:
                            ...
                            self.field[j][
                                i] = self.field[j][i] + self.field[k][i]
                            self.field[k][i] = 0
                            ...
                            break
                for j in range(0,
                               self.height):  #move all the blocks up in heap
                    if self.field[j][i] == 0:
        ...
        def transpose(
        ):  #transpose the matrix gamefield around its diagnal line
    ...
    def gameover(self, state):
        if state == 'Gameover':
            self.stdscr.addstr(
                os.linesep * 2 +
                'You have lost, press R to restart or Q to Exit')
        else:
            self.stdscr.addstr(
                os.linesep * 2 +
                'Congratulatons!! You have win, press R to restart or Q to Exit'
            )
        ch = chr(self.stdscr.getch())

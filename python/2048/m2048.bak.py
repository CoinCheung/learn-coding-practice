...
class GameField():
	def __init__(self,screen):
		...
		self.highscore=0
		self.stdscr=screen
		self.actiondict=dict(zip('wsadrqWSADRQ',('up','down','left','right','restart','quit')*2))
	...
	def draw(self):
		...
		self.stdscr.clear()
		self.stdscr.addstr('SCORE: {0}      HGHSCORE: {1}'.format(self.score,self.highscore)+os.linesep)
	...
	def move(self,direction):
		...
		def invert():#transpose the matrix around its counter diagnal line
			self.field.reverse()
			self.field=[[token[i] for token in self.field] for i in range(self.width-1,-1,-1)]
			self.height=self.height^self.width
			self.width=self.height^self.width
			self.height=self.height^self.width
	...
	def gameover(self,state):
		if state=='Gameover':
			self.stdscr.addstr(os.linesep*2+'You have lost, press R to restart or Q to Exit')
		else:
			self.stdscr.addstr(os.linesep*2+'Congratulatons!! You have win, press R to restart or Q to Exit')
		ch=chr(self.stdscr.getch())

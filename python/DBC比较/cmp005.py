...
'''
* fix the mistake of counting direction to check overlap
* add signal name prefix and postfix splitting feature
* fix the BA_DEFs mismatch error, and add BA_DEF_ merging feature
* refine the comment
*
...
####
## function: signalAll, messagesDictAll badefs = merge(signals1, signals2, messagesDict1, messagesDict2, tuple_badefs, dbcnames, logname)
##
## merge the two signal lists and message dicts to one each
##
## input: lists of sdicts of dbc1 and dbc2, dict of messages created from dbc1 and dbc2, tuple of BA_DEF lines (badefs1, badefs2)got from dbc1 and dbc2, tuple of dbc names: (dbcname1, dbcname2), and the logfile name
## return: merged list of sdicts got from dbc1 and dbc2, merged dict of mdicts got from dbc1 and dbc2, merged BA_DEF_ lines from dbc1 and dbc2
